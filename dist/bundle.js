...
(() => {
  ...
  // src/historyPanel.ts
  function renderHistoryPanel(container, history, onClose, onReplay) {
    container.innerHTML = "";
...
public record Points(string PointName, double XAnchor, double YAnchor);`;
  ...
  function renderExportControls(controlsEl, getPoints, onExport) {
...
${suffix}`;
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleExport(language, content) {
      if (currentUser) {
        saveExport(
          currentUser.id,
          "Untiteld Image",
    ...
    historyBtnEl.addEventListener("click", () => {
      ...
      getHistory(currentUser.id).then((history) => {
        ...
        renderHistoryPanel(
          ...
          (item) => {
            openExportModal(item.points_data, item.export_language);
          }
        );
      });
    });
